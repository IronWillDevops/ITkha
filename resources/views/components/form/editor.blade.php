    ...
    <div class="wysiwyg-modal-content">
        ...
        <div class="wysiwyg-modal-body">
            ...
            <div class="wysiwyg-form-group" id="modalTableGroup" style="display:none;">
                ...
                <input type="number" id="wysiwygModalRows" class="wysiwyg-input" min="1" max="20" value="3">
            </div>
            <div class="wysiwyg-form-group" id="modalTableColsGroup" style="display:none;">
                <label data-i18n="columns">Columns</label>
                <input type="number" id="wysiwygModalCols" class="wysiwyg-input" min="1" max="10" value="3">
            </div>
...
function wysiwygInsertBlock(e, type) {
    ...
    while (cb && cb !== ed) {
        if (cb.nodeType === 1 && cb.parentNode === ed) {
            break;
        }
        cb = cb.parentNode;
    }
    ...
    if (cb && cb !== ed && cb.parentNode === ed) {
        cb.parentNode.insertBefore(b, cb.nextSibling);
        cb.parentNode.insertBefore(np, b.nextSibling);
    } else {
        r.deleteContents();
...
function wysiwygOpenTableModal(e) {
    ...
    showM('insertTable', { url: 0, txt: 0, tgt: 0, aln: 0, tbl: 1, del: 0, delMedia: 0, delTable: 0, prv: 0 });
}
...
function wysiwygTableAddCol(btn, pos) {
    ...
    rows.forEach((row, idx) => {
        const cell = idx === 0 && row.querySelector('th') ? document.createElement('th') : document.createElement('td');
        cell.contentEditable = 'true';
        cell.innerHTML = '<br>';
...
function wysiwygEditTable(btn) {
    ...
    showM('editTable', { url: 0, txt: 0, tgt: 0, aln: 0, tbl: 0, del: 0, delMedia: 0, delTable: 1, prv: 0 });
}
