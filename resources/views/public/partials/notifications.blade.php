@if (session('success') || session('error'))
    <div id="toast-container" class="fixed bottom-4 right-4 z-50 flex flex-col space-y-4">
        @if (session('success'))
            <div
                class="toast fixed bottom-4 right-4 max-w-sm w-full bg-surface border border-border rounded-xl shadow-lg flex items-start p-4 gap-3 text-text-primary z-50 transition-transform duration-300 ease-in-out animate-slide-in-bottom">
