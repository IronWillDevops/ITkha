...
<nav aria-label="Pagination" class="flex justify-center mt-4">
    <ul class="inline-flex items-center gap-1.5 text-sm">
        @php
            $baseClasses = 'w-8 h-8 flex items-center justify-center border font-semibold rounded-sm focus:outline-none focus:ring focus-visible:ring-ring';
        ...
        <li>
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="{{ $baseClasses }} {{ $inactiveClasses }}" aria-label="Previous page">
                    <i class="fa-solid fa-angles-left"></i>
                </a>
            @endif
        </li>
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <li>
                            <a href="{{ $url }}" class="{{ $baseClasses }} {{ $inactiveClasses }}" aria-label="Go to page {{ $page }}">
                                {{ $page }}
                            </a>
                        </li>
                    @endif
        ...
        <li>
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="{{ $baseClasses }} {{ $inactiveClasses }}" aria-label="Next page">
