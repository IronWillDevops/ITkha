    ...
    <div class="mx-auto p-6 bg-surface border border-border rounded-lg text-text-primary">
        ...
        <form action="{{ route('admin.user.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {{-- Имя --}}
                <x-admin.form.input name="name" label="{{ __('admin/users.fields.name') }}" placeholder="{{ __('admin/users.placeholder.name') }}" icon="fas fa-solid fa-user" />

                {{-- Фамилия --}}
                <x-admin.form.input name="surname" label="{{ __('admin/users.fields.surname') }}" placeholder="{{ __('admin/users.placeholder.surname') }}"
                    icon="fas fa-solid fa-user" />

                {{-- Логин --}}
                <x-admin.form.input name="login" label="{{ __('admin/users.fields.login') }}"  placeholder="{{ __('admin/users.placeholder.login') }}"
                    icon="fas fa-solid fa-user" />


                {{-- Email --}}
                <x-admin.form.input type="email" name="email" label="{{ __('admin/users.fields.email') }}"  placeholder="{{ __('admin/users.placeholder.email') }}"
                    icon="fas fa-solid fa-at" />


                {{-- Пароль --}}
                <x-admin.form.input type="password" name="password" label="{{ __('admin/users.fields.password') }}" icon="fas fa-solid fa-lock"  placeholder="{{ __('admin/users.placeholder.password') }}"/>
