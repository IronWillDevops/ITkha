    ...
    <div class="mx-auto p-4 bg-surface border border-border rounded-lg text-text-primary">
        <div class="flex items-center gap-4 mb-4">
            <a href="{{ route('admin.role.edit', $role->id) }}"
                ...
                <i class="fas fa-edit"></i> {{ __('admin/roles.actions.edit') }}
            </a>
            ...
            <form action="{{ route('admin.role.delete', $role->id) }}" method="POST">
                ...
                <button type="submit" class="px-4 py-2 border border-border rounded-xl shadow transition text-text-danger"
                    ...
                    <i class="fas fa-trash-alt"></i> {{ __('admin/roles.actions.delete') }}
                </button>
            </form>
        </div>
                ...
                <table class="min-w-full divide-y ">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2 text-left text-sm font-semibold">{{ __('admin/users.fields.status') }}
                            </th>
                            ...
                            <th class="px-4 py-2 text-center text-sm font-semibold">{{ __('admin/common.actions.title') }}
                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y ">
                        @forelse($users as $user)
                            <tr>
                                ...
                                <td class="px-4 py-2 text-sm link hover:link-hover hover:underline "><a
                                        href="{{ route('admin.user.show', $user->id) }}">{{ $user->login }}</a></td>
                                <td class="px-4 py-2 text-sm ">{{ $user->email }}</td>
                                <td class="px-4 py-2 text-sm link hover:link-hover hover:underline"><a
                                        href="{{ route('admin.role.show', $user->roles->first()->id) }}">{{ $user->roles->first()?->title }}</a>
                                </td>
