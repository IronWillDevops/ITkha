    ...
    @section('admin.content')
        <div class="mx-auto p-6 bg-surface border border-border rounded-lg text-text-primary shadow-sm">
            ...
            <form action="{{ route('admin.post.update', $post->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <x-admin.form.input name="title" label="{{ __('admin/posts.fields.title') }}" value="{{ $post->title }}" placeholder="{{ __('admin/posts.placeholder.title') }}" icon="fa-solid fa-heading"/>
                ...
                {{-- Editor --}}
                <x-admin.form.editor name="content" label="{{ __('admin/posts.fields.content') }}"  placeholder="{{ __('admin/posts.placeholder.content') }}"
                    value="{{ $post->content }}" />
